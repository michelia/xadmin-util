...
class CertificateImageInput(forms.FileInput):
    ...
    def render(self, name, value, attrs=None):
        ...
        # }
        # template = u'%(input)s'
        # substitutions['input'] = super(CertificateImageInput, self).render(name, value, attrs)

        # # if value and hasattr(value, "url"):
        # template = self.template_with_initial
        # # substitutions['initial'] = (u'<a href="%s">%s</a>' % (escape(value.url), escape(force_unicode(value))))
        # # substitutions['initial'] = (u'<img src="%s"  width="150" height="150"></img>' % (escape(value.url)))
        # substitutions['initial'] = (u'<img src="%s"  width="150" height="150"></img>' % '')
        # # if not self.is_required:
        # if 1:
        #     checkbox_name = self.clear_checkbox_name(name)
        #     checkbox_id = self.clear_checkbox_id(checkbox_name)
        ...
        # 下面这种方式可以去掉后面再增加类
        attrs.update({'class': ''})
        template = u'<img src="%(img_uri)s"  width="150" height="150"></img>%(input)s'
        substitutions = {
